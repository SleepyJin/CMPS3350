	...
	for (int i=0; i<game->n; i++){
		...
		for (int j=1;j<5;j++){
			...
			if(dist <= game-> circle.radius){
				...
				p->velocity.x+=(d1/dist)*.25;
				p->velocity.y+=(d2/dist)*.25;
			}
		}
...
/*
	...
	ggprint8b(&r, 16, 0xFF0000, "Requirements");

	s= &game ->box[1];
	r.left = s->center.x - s->width+ 50;
	r.bot  = s->center.y - s->height;
	ggprint8b(&r, 16, 0xFF6600, "Design");

	s= &game ->box[2];
	r.left = s->center.x - s->width+ 50;
	r.bot  = s->center.y - s->height;
	ggprint8b(&r, 16, 0xFFFF00, "Coding");

	s= &game ->box[3];
	r.left = s->center.x - s->width+ 50;
	r.bot  = s->center.y - s->height;
	ggprint8b(&r, 16, 0x00CC00, "Testing");
